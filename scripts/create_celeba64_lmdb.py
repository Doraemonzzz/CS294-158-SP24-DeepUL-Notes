# copy from https://github.com/NVlabs/NVAE/blob/master/scripts/create_celeba64_lmdb.py
...
def main(split, img_path, lmdb_path):
    ...
    data = dset.CelebA(
        root=img_path, split=split, target_type="attr", transform=None, download=False
    )
    ...
    with env.begin(write=True) as txn:
        for i in range(len(data)):
            file_path = os.path.join(
                data.root, data.base_folder, "img_align_celeba", data.filename[i]
            )
            attr = data.attr[i, :]
